    ...
    try:
        ...
        for reaction in message.reactions:
            if str(reaction.emoji) == '✅':
                ...
                async for user in reaction.users():
                    if not user.bot:
                        helping_users.append(user)
        
        attending_text = ", ".join([user.mention for user in attending_users]) if attending_users else "No one yet"
        helping_text = ", ".join([user.mention for user in helping_users]) if helping_users else "No one yet"
        
        embed.set_field_at(2, name="✅ Attending", value=attending_text, inline=False)
        embed.set_field_at(3, name="🦅 Helping", value=helping_text, inline=False)
...
async def schedule_training(interaction: discord.Interaction, training_type: str, time: str):
    ...
    embed.add_field(name="Time", value=discord_timestamp, inline=True)
    embed.add_field(name="✅ Attending", value="No one yet", inline=False)
    embed.add_field(name="🦅 Helping", value="No one yet", inline=False)
...
@bot.tree.command(name="exportconfig", description="Export all bot settings to config.json file (Admin only)")
@app_commands.checks.has_permissions(administrator=True)
async def export_config(interaction: discord.Interaction):
    ...
    try:
        ...
        if welcome:
            config_data["welcome_config"].append({
                "channel_id": welcome.get('channel_id'),
                "message": welcome.get('message'),
                "auto_role_id": welcome.get('auto_role_id')
            })
        
        cur.execute('SELECT * FROM training_config WHERE guild_id = %s', (interaction.guild.id,))
        trainings = cur.fetchall()
        for training in trainings:
            config_data["training_config"].append({
                "training_type": training['training_type'],
                "channel_id": training.get('channel_id'),
                "message": training.get('message')
            })
        
        cur.execute('SELECT * FROM monthly_awards WHERE guild_id = %s', (interaction.guild.id,))
        awards = cur.fetchall()
        for award in awards:
            config_data["monthly_awards"].append({
                "award_type": award['award_type'],
                "channel_id": award.get('channel_id'),
                "message": award.get('message')
            })
        
        cur.execute('SELECT * FROM training_roles WHERE guild_id = %s', (interaction.guild.id,))
        helper_role = cur.fetchone()
        if helper_role:
            config_data["training_roles"].append({
                "helper_role_id": helper_role.get('helper_role_id')
            })
        
        cur.execute('SELECT * FROM global_log_config WHERE guild_id = %s', (interaction.guild.id,))
        global_log = cur.fetchone()
        if global_log:
            config_data["global_log_config"].append({
                "channel_id": global_log.get('channel_id'),
                "enabled": global_log.get('enabled')
            })
        
        cur.execute('SELECT * FROM security_config WHERE guild_id = %s', (interaction.guild.id,))
        security = cur.fetchone()
        if security:
            config_data["security_config"].append({
                "anti_raid_enabled": security.get('anti_raid_enabled'),
                "raid_threshold": security.get('raid_threshold'),
                "raid_time_window": security.get('raid_time_window'),
                "min_account_age": security.get('min_account_age'),
                "auto_lockdown": security.get('auto_lockdown'),
                "alert_role_id": security.get('alert_role_id'),
                "permission_guard_enabled": security.get('permission_guard_enabled')
            })
        
        cur.execute('SELECT * FROM duty_role_config WHERE guild_id = %s', (interaction.guild.id,))
        duty_role = cur.fetchone()
        if duty_role:
            config_data["duty_role_config"].append({
                "on_duty_role_id": duty_role.get('on_duty_role_id')
            })
        
        cur.execute('SELECT * FROM presence_config LIMIT 1')
        presence = cur.fetchone()
        if presence:
            config_data["presence_config"].append({
                "activity_type": presence.get('activity_type'),
        ...
        await interaction.followup.send('✅ Configuration exported to `config.json`! This file will be included when you move your bot to a new project.', ephemeral=True)
        
    except Exception as e:
        print(f'Error exporting config: {e}')
        await interaction.followup.send(f'❌ Error exporting configuration: {str(e)}', ephemeral=True)

@bot.tree.command(name="importconfig", description="Import settings from config.json file (Admin only)")
@app_commands.checks.has_permissions(administrator=True)
async def import_config(interaction: discord.Interaction):
    ...
    try:
        if not os.path.exists('config.json'):
            await interaction.followup.send('❌ No config.json file found!', ephemeral=True)
            return
        ...
        if config_data.get("welcome_config"):
            for welcome in config_data["welcome_config"]:
                cur.execute('''
                    INSERT INTO welcome_config (guild_id, channel_id, message, auto_role_id)
                    VALUES (%s, %s, %s, %s)
                    ON CONFLICT (guild_id) DO UPDATE 
                    SET channel_id = %s, message = %s, auto_role_id = %s
                ''', (interaction.guild.id, welcome.get('channel_id'), welcome.get('message'), welcome.get('auto_role_id'),
                      welcome.get('channel_id'), welcome.get('message'), welcome.get('auto_role_id')))
                imported_count += 1
        
        if config_data.get("training_config"):
            for training in config_data["training_config"]:
                cur.execute('''
                    INSERT INTO training_config (guild_id, training_type, channel_id, message)
                    VALUES (%s, %s, %s, %s)
                    ON CONFLICT (guild_id, training_type) DO UPDATE 
                    SET channel_id = %s, message = %s
                ''', (interaction.guild.id, training['training_type'], training.get('channel_id'), training.get('message'),
                      training.get('channel_id'), training.get('message')))
                imported_count += 1
        
        if config_data.get("monthly_awards"):
            for award in config_data["monthly_awards"]:
                cur.execute('''
                    INSERT INTO monthly_awards (guild_id, award_type, channel_id, message)
                    VALUES (%s, %s, %s, %s)
                    ON CONFLICT (guild_id, award_type) DO UPDATE 
                    SET channel_id = %s, message = %s
                ''', (interaction.guild.id, award['award_type'], award.get('channel_id'), award.get('message'),
                      award.get('channel_id'), award.get('message')))
                imported_count += 1
        
        if config_data.get("training_roles"):
            for role in config_data["training_roles"]:
                cur.execute('''
                    INSERT INTO training_roles (guild_id, helper_role_id)
                    VALUES (%s, %s)
                    ON CONFLICT (guild_id) DO UPDATE SET helper_role_id = %s
                ''', (interaction.guild.id, role.get('helper_role_id'), role.get('helper_role_id')))
                imported_count += 1
        
        if config_data.get("global_log_config"):
            for log in config_data["global_log_config"]:
                cur.execute('''
                    INSERT INTO global_log_config (guild_id, channel_id, enabled)
                    VALUES (%s, %s, %s)
                    ON CONFLICT (guild_id) DO UPDATE 
                    SET channel_id = %s, enabled = %s
                ''', (interaction.guild.id, log.get('channel_id'), log.get('enabled', True),
                      log.get('channel_id'), log.get('enabled', True)))
                imported_count += 1
        
        if config_data.get("security_config"):
            for sec in config_data["security_config"]:
                cur.execute('''
                    INSERT INTO security_config (guild_id, anti_raid_enabled, raid_threshold, raid_time_window, 
                                                min_account_age, auto_lockdown, alert_role_id, permission_guard_enabled)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                    ON CONFLICT (guild_id) DO UPDATE 
                    SET anti_raid_enabled = %s, raid_threshold = %s, raid_time_window = %s, 
                        min_account_age = %s, auto_lockdown = %s, alert_role_id = %s, permission_guard_enabled = %s
                ''', (interaction.guild.id, sec.get('anti_raid_enabled'), sec.get('raid_threshold'), 
                      sec.get('raid_time_window'), sec.get('min_account_age'), sec.get('auto_lockdown'), 
                      sec.get('alert_role_id'), sec.get('permission_guard_enabled'),
                      sec.get('anti_raid_enabled'), sec.get('raid_threshold'), sec.get('raid_time_window'), 
                      sec.get('min_account_age'), sec.get('auto_lockdown'), sec.get('alert_role_id'), 
                      sec.get('permission_guard_enabled')))
                imported_count += 1
        
        if config_data.get("duty_role_config"):
            for duty in config_data["duty_role_config"]:
                cur.execute('''
                    INSERT INTO duty_role_config (guild_id, on_duty_role_id)
                    VALUES (%s, %s)
                    ON CONFLICT (guild_id) DO UPDATE SET on_duty_role_id = %s
                ''', (interaction.guild.id, duty.get('on_duty_role_id'), duty.get('on_duty_role_id')))
                imported_count += 1
        
        if config_data.get("presence_config"):
            for pres in config_data["presence_config"]:
                cur.execute('''
                    INSERT INTO presence_config (guild_id, activity_type, status_message, last_updated)
                    VALUES (%s, %s, %s, %s)
                    ON CONFLICT (guild_id) DO UPDATE 
                    SET activity_type = %s, status_message = %s, last_updated = %s
                ''', (interaction.guild.id, pres.get('activity_type'), pres.get('status_message'), datetime.now(),
                      pres.get('activity_type'), pres.get('status_message'), datetime.now()))
                imported_count += 1
        ...
        await interaction.followup.send(f'✅ Configuration imported successfully! {imported_count} settings restored.', ephemeral=True)
        
    except Exception as e:
        print(f'Error importing config: {e}')
        await interaction.followup.send(f'❌ Error importing configuration: {str(e)}', ephemeral=True)
